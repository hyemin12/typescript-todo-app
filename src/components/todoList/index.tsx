...
function TodoList({ setWelcomeTip }: TodoListProps) {
  ...
  const filteredTodo = ({ todos, filterType }: { todos: TodosProps; filterType: string }) => {
    ...
    return (
      <div className={styles.empty_todo}>
        ...
        <div className={styles.tool_tip_wrapper}>
          <p>todo가 처음이신가요? </p>
          <p className={styles.tool_tip_btn} role='button' onClick={openWelcomeTip}>
            사용방법 알아보기
          </p>
        </div>
      </div>
  ...
  if (filteredTodos.length === 0)
    return (
      <div className={styles.empty_todo}>
        <h4>{filterType === "active" ? "진행 중인" : "완료된"}할 일이 없습니다.</h4>
      </div>
    );
  return (
